...
const loadForms = async () => {
  loading.value = true
  const res = await useFetch('http://localhost:3001/api/form-admin')
  console.log('Ответ от сервера:', res.data.value)
  forms.value = res.data.value?.data || []
  loading.value = false
}
const loadForms = async () => {
  loading.value = true
  ...
  // ❗Проверь, что здесь data - массив
  if (Array.isArray(json.data)) {
    forms.value = json.data
  } else {
    console.warn('Формат данных неверный:', json.data)
    forms.value = []
  }
