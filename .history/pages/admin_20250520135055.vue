...
const loadForms = async () => {
  loading.value = true
  const formRes = await fetch('http://localhost:3001/api/form-admin')
  const briefRes = await fetch('http://localhost:3001/api/brief-admin')
  const json = await formRes.json()
  const json1 = await briefRes.json()
  console.log('Ответ от API:', json)
  forms.value = json.data
  forms.value = json1.data
