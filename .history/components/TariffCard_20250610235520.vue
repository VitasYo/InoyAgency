<template>
  <div 
    class="tariff-card" 
    :class="`tariff-card--${tariff.color}`"
    @click="$emit('click')"
  >
    <h3 class="tariff-title">{{ tariff.title }}</h3>
    <div class="tariff-price">{{ tariff.price }}</div>
    <ul class="tariff-features">
      <li v-for="(feature, index) in tariff.features" :key="index">
        {{ feature }}
      </li>
    </ul>
    <div class="tariff-button">Подробнее</div>
  </div>
</template>

<script lang="ts">
import { defineComponent, PropType } from 'vue'
import { Tariff } from '@/pages/services/full.vue' // Или путь к вашему интерфейсу
